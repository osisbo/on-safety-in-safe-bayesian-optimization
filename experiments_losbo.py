...
def calculate_L_nom(lengthscale, noise_bound, B_nom):
    ...
    def sample_fun():
        return safeopt.utilities.sample_function_with_fixed_B(kernel, bounds, noise_bound, grid_points_per_axis,
                                                              B_nom=B_nom)
...
def run_exp(num_functions, factor_L, factor_B, lengthscale, noise_bound, noise_std, B, L_nom, used_random_seed):
    safety_threshold_exploration = 0
    ...
    def get_fun_with_safe_grid_seeds_np():
        job_done = False
        while not job_done:
            fun = safeopt.utilities.sample_function_with_fixed_B(kernel, bounds, noise_bound, grid_points_per_axis,
                                                                 B_nom=B_nom)
    ...
    experiment_data = [("fiedler", "lipschitz"), ("chowdhury", "lipschitz"), ("fiedler-lipschitz", "pure-lipschitz")]
    ...
    result_dict = {}
    for style, safety_style in experiment_data:
    ...
    for i in range(num_functions):
        ...
        for j, index_x0 in enumerate(safe_indices):
            ...
            for style, safety_style in experiment_data:
                y0 = fun(x0)
                gp = GPy.models.GPRegression(x0, y0, kernel, noise_var=noise_variance)
                beta_dict = {"style": style, "B": B_ass, "R": noise_bound, "delta": 0.01, "lambda": 1,
                             "noise_variance": noise_variance, "card_D": card_D, "safety": safety_style,
                             "index_x0": index_x0, "y0": y0}
                ...
                for k in range(number_iterations_optimization):
                    ...
                    try:
                        ...
                        opt.add_new_data_point(x_next, index_x, y_meas)
    ...
    dir_path = store_path + "/" + log_time + "-" + str(factor_L) + "-" + str(factor_B) + "-" + str(
        lengthscale) + "-" + str(noise_bound) + "-" + str(B) + "-" + str(worker_id)
    ...
    for counter, (style, safety_style) in enumerate(experiment_data):
    ...
    print("p-value cleaned comparison between fiedler and chowdhurry (based on sample regret): {0}".format(p_value))
    ...
    with open(dir_path + "/" + "safeopt_results.txt", "a") as f:
        ...
        print("noise_bound: ", noise_bound, file=f)
        print("noise_variance: ", noise_variance, file=f)
...
if __name__ == "__main__":
    ...
    predefined_safe_seeds_02_2 = [1652870715, 1652870717, 1652870720, 1652870722, 1652870716, 1652870718, 1652870714,
                                  1652870721, 1652870723, 1652870719]
    predefined_safe_seeds_02_20 = [1652870871, 1652870873, 1652870868, 1652870876, 1652870869, 1652870874, 1652870875,
                                   1652870877, 1652870872, 1652870870]
    predefined_safe_seeds_05_2 = [1652876427, 1652876433, 1652876435, 1652876430, 1652876426, 1652876432, 1652876429,
                                  1652876431, 1652876428, 1652876434]
    predefined_safe_seeds_05_20 = [1652869129, 1652869123, 1652869121, 1652869125, 1652869124, 1652869122, 1652869126,
                                   1652869128, 1652869127, 1652869130]
    L_02_2 = 12
    L_02_20 = 119
    L_05_2 = 6
    L_05_20 = 57
    ...
    for i in range(num_workers):
        pool.apply_async(run_exp,
                         args=(
                             num_functions, factor_L, factor_B, lengthscale, noise_bound, noise_std,
                             B_nom, L_nom, seed_list[i],))
